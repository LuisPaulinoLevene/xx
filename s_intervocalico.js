...
function gerarPlano() {
    ...
    if (escola && data && nome && disciplina && classe && turma && unidadeTematica && tema && objetivosText && tempo && duracao && tipoDeAula) {
        ...
        if (conteudoMediacao.trim() !== '') {
            ...
            rows[1].cells[2].innerHTML = `<div class="left-align">- Orienta abrir pag. ${conteudoMediacao}; - Orienta a observação da imagem; - Orienta a leitura e interpretação da imagem; - Orienta a identificação da frase chave; - Escreve a frase chave no quadro; - Orienta leitura da frase chave; - Orienta a identificação da palavra ${palavraChave}; -Orienta a identificação do "s" intervocálico; Explicação da leitura de "s" quando está entre vogais- Orienta os alunos a dizerem palavras que possuem "s" intervocálico.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[2].innerHTML = `<div class="left-align">- Ilustra o (a) ${materialDidatico}; - Orienta a observação; - Orienta a leitura e interpretação; - Orienta a identificação da frase chave; - Escreve a frase chave no quadro; - Orienta leitura da frase chave; - Orienta a identificação da palavra ${palavraChave};  -Orienta a identificação do "s" intervocálico; -Explica que o "s" quando está entre vogais lê-se como "z"- Orienta os alunos a dizerem palavras que possuem "s" intervocálico.</div>`;
        }

        // Verifica se conteudoMediacao está preenchido
        if (conteudoMediacao.trim() !== '') {
            // Se preenchido, exibe o conteúdo"
            rows[1].cells[3].innerHTML = `<div class="left-align">- Abre a pag. ${conteudoMediacao}; - Observa a imagem; - Lê e interpreta a imagem respondendo as perguntas; - Identifica a frase chave; - Lê repetidamente a frase chave; - Identifica a palavra ${palavraChave};  -Identifica o "s" intervocálico; -Pronucia correctamente o "s" quando está entre vogais- Orienta os alunos a dizerem palavras que possuem "s" intervocálico.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[3].innerHTML = `<div class="left-align">- Observa o (a) ${materialDidatico}; - Identtifica e interpreta o que vê; - Identifica e lê a frase chave; <br>- Lê repetidamente a frase chave; - Identifica a palavra ${palavraChave}; - Identifica a dupla ${letraEstudo}; - Diz palavras que entram dupla ${letraEstudo}.</div>`;
        }
