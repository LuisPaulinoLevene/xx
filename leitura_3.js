...
function gerarPlano() {
    ...
    if (escola && data && nome && disciplina && classe && turma && unidadeTematica && tema && objetivosText && tempo && duracao && tipoDeAula) {
        ...
        if (conteudoMediacao) {
            rows[1].cells[1].innerHTML = `<div class="left-align"><strong>${tema}</strong>. <br><br> Leitura e interpretação da imagem;- Leitura modelo; -Leitura acompanhada; -Leitura em grupo e leitura individual.</div>`;
        } else {
            rows[1].cells[1].innerHTML = `<div class="left-align"><strong>${tema}</strong>. <br><br> Leitura e interpretação da imagem; -Leitura modelo; Leitura acompanhada; Leitura em grupo e leitura individual</div>`;
        }
        rows[1].cells[2].innerHTML = `<div class="left-align">-Orienta a abrir na pagina ${conteudoMediacao} do livro do aluno;-Orienta a leitura e interpretação da imagem, -Lê em voz alta; Orienta os alunos a repitir o que o professor lê; Orienta a leitura em grupo; Orienta a leitura individual.</div>`;
        rows[1].cells[3].innerHTML = `<div class="left-align">-Abre a pagina ${conteudoMediacao} do livro do aluno; -Lê e interpreta a imagem, -Acompanha a leitura do professor; -Repete o que o professor lê; -Lê em grupo; Lê individualmente.</div>`;
        ...
        rows[2].cells[1].innerHTML = `<div class="left-align">Exercícios  de interpretação. <br> <br> ${conteudoDominio} </div>`;
