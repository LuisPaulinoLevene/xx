...
function gerarPlano() {
    ...
    if (escola && data && nome && disciplina && classe && turma && unidadeTematica && tema && objetivosText && tempo && duracao && tipoDeAula) {
        ...
        document.getElementById('infoTema').innerHTML = `Tema: <strong>Exercicios de aplicação sobre ${tema}</strong>.`;
        ...
        if (conteudoMediacao) {
            rows[1].cells[1].innerHTML = `<div class="left-align"><strong>Resumo da aula sobre ${tema}</strong>. <br><br> Veja o resumo na página ${conteudoMediacao} do livro do aluno.</div>`;
        } else {
            rows[1].cells[1].innerHTML = `<div class="left-align"><strong>Resumo da aula sobre ${tema}</strong>. <br><br>Resumo em anexo.</div>`;
        }
        rows[1].cells[2].innerHTML = `<div class="left-align">- Orienta o resumo da aula relacionada a ${tema}.</div>`;
        rows[1].cells[3].innerHTML = `<div class="left-align">- Resume a aula relacionada a ${tema}.</div>`;
