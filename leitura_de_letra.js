...
function gerarPlano() {
    ...
    const silabaChave = document.getElementById('silabaChave').value;
    ...
    if (escola && data && nome && disciplina && classe && turma && unidadeTematica && tema && objetivosText && tempo && duracao && tipoDeAula && materialDidatico && palavraChave && silabaChave && letraEstudo) {
        ...
        rows[0].cells[1].innerHTML = `<div class="left-align">- Correcção do TPC; <br>- Apresentação do tema.</div>`;
        rows[0].cells[2].innerHTML = `<div class="left-align">- Corrige o TPC;<br>- Passa o tema no quadro.</div>`;
        rows[0].cells[3].innerHTML = `<div class="left-align">- presta atenção na correcção do TPC;<br>- Passa o tema no caderno;</div>`;
        ...
        if (conteudoMediacao.trim() !== '') {
            ...
            rows[1].cells[2].innerHTML = `<div class="left-align">- Explora os conhecimentos; - Orienta abrir pag. ${conteudoMediacao}; - Orienta a observação da imagem; - Orienta a leitura e interpretação da imagem; - Orienta a identificação da frase chave; - Escreve a frase chave no quadro; - Orienta leitura da frase chave; - Orienta a identificação da palavra ${palavraChave}; - Orienta a divisão silabica da palavra ${palavraChave}; - Orienta a identificação da silaba ${silabaChave}; - Orienta a identificação da letra ${letraEstudo}; - Orienta a leitura da letra ${letraEstudo}; - Ilustra as formas da letra ${letraEstudo}.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[2].innerHTML = `<div class="left-align">- Explora os conhecimentos; - Ilustra o (a) ${materialDidatico}; - Orienta a observação; - Orienta a leitura e interpretação; - Orienta a identificação da frase chave; - Escreve a frase chave no quadro; - Orienta leitura da frase chave; - Orienta a identificação da palavra ${palavraChave}; - Orienta a divisão silabica da palavra ${palavraChave}; - Orienta a identificação da silaba ${silabaChave}; - Orienta a identificação da letra ${letraEstudo}; - Orienta a leitura da letra ${letraEstudo}; - Ilustra as formas da letra ${letraEstudo}.</div>`;
        }

        // Verifica se conteudoMediacao está preenchido
        if (conteudoMediacao.trim() !== '') {
            // Se preenchido, exibe o conteúdo"
            rows[1].cells[3].innerHTML = `<div class="left-align">- Contribui pelos conhecimentos sobre o tema; - Abre a pag. ${conteudoMediacao}; - Observa a imagem; - Lê e interpreta a imagem respondendo as perguntas; - Identifica a frase chave; - Lê repetidamente a frase chave; - Identifica a palavra ${palavraChave}; - Divide em silabas a palavra ${palavraChave} batendo palmas; - Identifica a silaba ${silabaChave}; - identifica a letra ${letraEstudo}; - Lê repetidamente a letra ${letraEstudo}; - Vê as 4 formas da letra ${letraEstudo}.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[3].innerHTML = `<div class="left-align">- Contribui pelos conhecimentos sobre o tema; - Observa o (a) ${materialDidatico}; - Identtifica e interpreta o que vê; - Identifica e lê a frase chave; <br>- Lê repetidamente a frase chave; - Identifica a palavra ${palavraChave};<br>- Divide em silabas a palavra ${palavraChave} batendo palmas; - Identifica a silaba ${silabaChave};- identifica a letra ${letraEstudo};- Lê repetidamente a letra ${letraEstudo}; - Vê as 4 formas da letra ${letraEstudo}.</div>`;
        }
