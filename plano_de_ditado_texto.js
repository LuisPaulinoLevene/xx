...
function gerarPlano() {
    ...
    const conteudoControle = document.getElementById('conteudoControle').value;
     const vezesJogo = document.getElementById('vezesJogo').value;
    ...
    if (escola && data && nome && disciplina && classe && turma && unidadeTematica && tema && objetivosText && tempo && duracao && tipoDeAula) {
        ...
        if (conteudoMediacao) {
            rows[1].cells[1].innerHTML = `<div class="left-align"><strong>${tema}</strong>. <br><br> Veja o texto na página ${conteudoMediacao} do livro do aluno.</div>`;
        } else {
            rows[1].cells[1].innerHTML = `<div class="left-align"><strong>${tema}</strong>. <br><br>Apontamentos em anexo.</div>`;
        }


        rows[1].cells[2].innerHTML = `<div class="left-align">- Passa o texto que se pretendem ditar no quadro; -Orienta a leitura; Orienta os alunos a tapar as vistas com livro, repetindo o processo ate ${vezesJogo} vezes; Apaga uma palavra a cada vez que manda tapar as vistas; Indica um aluno para identificar e escrever a palavra em falta a cada vez que apaga uma palavra; Explica as regras de escrita; -Limpa o quadro; -Dita as frases pausadamente repetindo no mínimo duas vezes.</div>`;
        rows[1].cells[3].innerHTML = `<div class="left-align">- Observa o texto escrito no quadro; -Lê o texto; tapa as vistas com livro sob orientação do professor; -Identifica a palavra em falta escreve-a; -Presta atenção na explicação das regras de escrita; -Escreve o que o professor dita.</div>`;
        ...
        rows[2].cells[1].innerHTML = `<div class="left-align">Exercícios  de aplicação. <br> <br> Leitura das frases escritas por cada aluno.</div>`;
        rows[2].cells[2].innerHTML = `<div class="left-align"> -Orienta a leitura do texto escrito por cada aluno.;</div>`;
        rows[2].cells[3].innerHTML = `<div class="left-align"> -Lê o texto escrito.</div>`;
