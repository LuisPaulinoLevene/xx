...
function gerarPlano() {
    ...
    if (escola && data && nome && disciplina && classe && turma && unidadeTematica && tema && objetivosText && tempo && duracao && tipoDeAula) {
        ...
        if (conteudoMediacao.trim() !== '') {
            ...
            rows[1].cells[2].innerHTML = `<div class="left-align">- Mostra o que se pretende copiar na pag. ${conteudoMediacao} do livro do aluno; -Orienta a leitura; Explica as regras de escrita; - Orienta a copia no caderno do aluno.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[2].innerHTML = `<div class="left-align"> -Escreve o que se pretende copiar no quadro; -Orienta a leitura; Explica as regras de escrita; - Orienta a copia no caderno do aluno.</div>`;
        }

        // Verifica se conteudoMediacao está preenchido
        if (conteudoMediacao.trim() !== '') {
            // Se preenchido, exibe o conteúdo"
            rows[1].cells[3].innerHTML = `<div class="left-align">- Observa o que se pretende copiar na pag. ${conteudoMediacao} do livro do aluno; -Lê; -Presta atenção na explicação das regras de escrita; -Copia no caderno do aluno.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[3].innerHTML = `<div class="left-align">  -Observa a escrita no quadro; -Lê no quadro; -Presta atenção na explicação das regras de escrita; -Copia as frases no caderno do aluno.</div>`;
        }
