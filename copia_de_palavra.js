...
function gerarPlano() {
    ...
    if (escola && data && nome && disciplina && classe && turma && unidadeTematica && tema && objetivosText && tempo && duracao && tipoDeAula) {
        ...
        rows[0].cells[3].innerHTML = `<div class="left-align"> -Recapitula a aula anterior; Anota o tema.</div>`;
        ...
        if (conteudoMediacao.trim() !== '') {
            ...
            rows[1].cells[2].innerHTML = `<div class="left-align">- Mostra as palavras no livro da pag. ${conteudoMediacao}; -Escreve-as no quadro; -Orienta a leitura; -Explica as regras de escrita;- Orienta a copia no livro do aluno.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[2].innerHTML = `<div class="left-align"> -Escreve as palvras no quadro; -Orienta a leitura; -Explica as regras de escrita;- Orienta a copia no caderno do aluno.</div>`;
        }

        // Verifica se conteudoMediacao está preenchido
        if (conteudoMediacao.trim() !== '') {
            // Se preenchido, exibe o conteúdo"
            rows[1].cells[3].innerHTML = `<div class="left-align">- Observa as palavras no livro da pag. ${conteudoMediacao}; - Observa a escrita no quadro; -Lê as palavras no quadro; -Presta atenção na explicação das regras de escrita; -Copia as palavras no livro do aluno.</div>`;
        } else {
            // Se não preenchido, exibe o conteúdo
            rows[1].cells[3].innerHTML = `<div class="left-align"> -Observas a escrita das palvras no quadro; -Lê as palavras no quadro; -Presta atenção na explicação das regras de escrita; -Copia as palavras no livro do aluno.</div>`;
        }
